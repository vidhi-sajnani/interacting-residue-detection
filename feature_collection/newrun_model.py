...
parser = argparse.ArgumentParser(description='Predict Secondary Structure with the S4PRED model',
                                 epilog='Takes a FASTA file containing an arbitrary number of individual sequences and outputs a secondary structure prediction for each.')
...
parser.add_argument('-o', '--outdir', metavar='p', type=str,
...
# Manually listing for clarity and hot swapping in the future
# Inelegant, ugly ugly, to be cleaned up in the future
s4pred.model_1.load_state_dict(
    torch.load(scriptdir + weight_files[0], map_location=lambda storage, loc: storage))
s4pred.model_2.load_state_dict(
    torch.load(scriptdir + weight_files[1], map_location=lambda storage, loc: storage))
s4pred.model_3.load_state_dict(
    torch.load(scriptdir + weight_files[2], map_location=lambda storage, loc: storage))
s4pred.model_4.load_state_dict(
    torch.load(scriptdir + weight_files[3], map_location=lambda storage, loc: storage))
s4pred.model_5.load_state_dict(
    torch.load(scriptdir + weight_files[4], map_location=lambda storage, loc: storage))
...
def format_ss2(data, ss, ss_conf):
    ...
    # Extract PDB ID from the header and remove any leading '>'
    pdb_id = data[0].split()[0][1:]
    for i in range(len(ss)):
        lines.append("%4d %c %c  %s %6.3f %6.3f" % (i + 1, data[2][i], ind2char[ss[i]], pdb_id, ss_conf[i, 1], ss_conf[i, 2]))
    return lines
...
def format_fas(data, ss, ss_conf, include_conf=False):
    ...
    if include_conf:
        lines.append(np.array2string(ss_conf[:, 0], max_line_width=1e6, precision=3,
                                     formatter={'float_kind': lambda x: "%.3f" % x}).replace('[', '').replace(']', ''))
        lines.append(np.array2string(ss_conf[:, 1], max_line_width=1e6, precision=3,
                                     formatter={'float_kind': lambda x: "%.3f" % x}).replace('[', '').replace(']', ''))
        lines.append(np.array2string(ss_conf[:, 2], max_line_width=1e6, precision=3,
                                     formatter={'float_kind': lambda x: "%.3f" % x}).replace('[', '').replace(']', ''))
...
def get_uniprot_id(header):
    # Assuming FASTA header format like ">UniProtID Some description"
    parts = header.split()
    if len(parts) > 0:
        return parts[0][1:]  # Remove the ">" symbol
    else:
        return ""
...
for idx, data in enumerate(seqs):
    ...
    uniprot_id = get_uniprot_id(data[0])
    ...
    if not args_dict['silent']:
        # Print UniProt ID before the secondary structure information
        print(">", uniprot_id)
        for line in lines:
            print(line)
    else:
    ...
    if args_dict['save_files']:
        if args_dict['save_by_idx']:
            ...
            if args_dict['prefix']:
                ...
                file_name = uniprot_id + suffix  # Use UniProt ID as the filename
        
        file_path = output_dir + file_name
        
        with open(file_path, 'w') as f:
            # Write UniProt ID to the file
            f.write("UniProt ID: " + uniprot_id + "\n")
            for line in lines:
                f.write(line + '\n')
